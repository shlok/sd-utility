...
testMinMax desc = testProperty desc $
  monadicIO $ do
    ...
    let min' = MMSeq.lookup mmSeqMin
    let max' = MMSeq.lookup mmSeqMax
    let fullMin = MMSeq.lookupFull mmSeqMin
    let fullMax = MMSeq.lookupFull mmSeqMax
    case length elements of
      0 -> return $ all isNothing [min', max', fullMin, fullMax]
      n
        | n < mmSeqMaxLen ->
          return $
            fromJust min' == minimum elements
              && fromJust max' == maximum elements
              && isNothing fullMin
              && isNothing fullMax
      _ ->
        let remElems = drop (length elements - mmSeqMaxLen) elements
            elemsMin = minimum remElems
            elemsMax = maximum remElems
         in return $
              fromJust min' == elemsMin
